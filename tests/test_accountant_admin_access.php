...
bdta_assert_contains($config, "\$_SESSION['admin_account_type'] = is_array(\$admin_user) ? \$admin_user['account_type'] : 'standard';", 'Admin config should refresh the current session account type from the database.');
...
bdta_assert_contains($invoicesView, "if (\$submitted_csrf_token === '' || !hash_equals(\$csrf_token_value, \$submitted_csrf_token)) {", 'Invoice send handler should validate the CSRF token.');
