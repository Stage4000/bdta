...
class CronRunner {
    ...
    private function parseCronExpression($cron) {
        ...
        if (is_numeric($minute) && $this->areAllWildcards([$hour, $day, $month, $weekday])) {
            ...
            $current_day = intval(gmdate('j'));
            $current_year = intval(gmdate('Y'));
            
            if ($current_minute < $target_minute) {
                // Later this hour
                $next = gmmktime(intval(gmdate('H')), $target_minute, 0, $current_month, $current_day, $current_year);
            } else {
                // Next hour
                $next = gmmktime(intval(gmdate('H')) + 1, $target_minute, 0, $current_month, $current_day, $current_year);
            }
            return gmdate('Y-m-d H:i:s', $next);
        }
        ...
        if (is_numeric($minute) && is_numeric($hour) && $this->areAllWildcards([$day, $month, $weekday])) {
            ...
            $current_day = intval(gmdate('j'));
            $current_year = intval(gmdate('Y'));
            $today_run = gmmktime($target_hour, $target_minute, 0, $current_month, $current_day, $current_year);
