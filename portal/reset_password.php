...
if (empty($token)) {
    ...
    $stmt = $conn->prepare("SELECT id, name, email FROM clients WHERE password_reset_token = ? AND password_reset_expires > datetime('now') AND (is_admin = 0 OR is_admin IS NULL)");
    $stmt->execute([$token]);
