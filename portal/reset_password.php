...
if (empty($token)) {
    ...
    $stmt = $conn->prepare("SELECT id, name, email FROM clients WHERE password_reset_token = ? AND password_reset_expires > CURRENT_TIMESTAMP AND (is_admin = 0 OR is_admin IS NULL) AND COALESCE(is_archived, 0) = 0");
    $stmt->execute([$token]);
