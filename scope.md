...
**Note:** There will be no client login system in Phase 1. All client interactions will occur via secure, direct-access links (booking, forms, contracts, quotes, invoices).
...
**Note:** Internal notes are always private, timestamped, and attached to the client (not pet-specific).
...
**Note:** Appointments may involve one or multiple pets.
...
**Note:** Group classes are included in Phase 1.
...
**Booking flow:**
1. Client selects appointment type
2. System checks availability (CRM + Google Calendar)
3. Client enters:
   - Pets involved
   - Reason for appointment
   - Brief pet history
4. System enforces required forms/contracts (if applicable)
5. Appointment is confirmed

**Note:** Some appointment types (e.g., consultations) require forms before booking.
...
**Note:** Internal notes do not sync.
...
**Note:** Invoices are generally due after sessions, except where configured otherwise.
...
**Note:** Invoice payment does not block booking by default.
...
**Note:** Reminder emails should include a calendar event link clients can add to their own calendar.
...
**Note:** Email delivery is required; SMS is optional.
